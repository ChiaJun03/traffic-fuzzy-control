...
class Fuzzy:
    ...
    def get_extension(self, arriving_green_light_car, behind_red_light_car, extension_count):
        ...
        # Rule 3: If Arrival is small AND Queue is (medim OR many) then Extension is short.
        # Rule 4: If Arrival is medium AND Queuse is (few OR small) then Extension is long.
        # Rule 5: If Arrival is medium AND Queue is (medim OR many) then Extension is medium.
        ...
        extension_activation_short = np.fmin(np.fmax(rule2, rule3), self.extension_short)
        extension_activation_medium = np.fmin(np.fmax(rule5, rule7), self.extension_medium)
        extension_activation_long = np.fmin(np.fmax(rule4, rule6), self.extension_long)
